...
当前 Agent 相关 pytest 已新增 trader 专项 18 个；项目已补齐 `scikit-learn` lock，最近一次全量后端测试为 `965 passed, 8 skipped, 0 failed`，前端 Vitest 为 `195 passed, 0 failed`。Backend CI #22 与 Frontend CI #28 均已通过。
...
- `services/agent/database_tools.py` 的查询入口已使用 `sqlglot` AST 做单语句、只读节点、危险函数和表白名单校验；用户私有数据仍通过显式 `user_id` 过滤，后续继续推进 AST 级过滤注入。
