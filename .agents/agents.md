...
Agent 系统按「功能能力」拆分，每个 Agent 有清晰边界、稳定输入输出、可测试的工具调用链路。当前已完成 Phase 0~2 与体验修复计划 P0/P1：DataAgent、DataQualityAgent、TechAnalysisAgent、RiskManagementAgent、AnalysisPipelineAgent、StrategyCompilerAgent、BacktestAgent、FactorMiningAgent 均已上线并接入真实进度流式；OrchestratorAgent 已从能力列表中移除，避免展示悬空能力。
...
4. **流式体验优先**：本地确定性 Agent（tech_analysis / risk_management / backtest / factor_mining / strategy_compiler / analysis_pipeline / data_quality）的 `run_stream` 通过后台任务 + 进度队列实时 yield 步骤/进度事件，避免「执行完再回放」的伪流式。
...
- `services/agent/executor.py`：执行引擎，负责任务创建、步骤持久化、流式事件发射；步骤由 per-step `commit()` 改为批量提交，消除 SQLite `database is locked`
...
当前 Agent 相关 pytest 65 个全部通过，后端全量测试 `669 passed, 7 skipped, 0 failed`。
...
- `frontend/app/chat/page.tsx`：9 种模式切换的 Chat 界面，支持流式展示 Agent 执行步骤；新增 `progress` 事件处理、`AbortController` 取消、停止按钮
...
- `services/agent/database_tools.py` 的 SQL 安全加固（使用 parser/参数化注入替代正则白名单）为 P2 远期项，不影响当前产品形态上线。
