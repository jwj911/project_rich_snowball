...
@router.get("/by-product-id/{product_id}", response_model=VarietyDetailResponse)
def get_variety_by_product_id(
    product_id: int,
    comment_skip: int = Query(0, ge=0),
    comment_limit: int = Query(100, ge=1, le=1000),
    db: Session = Depends(get_db),
    ...
    """通过 ProductDB 的 id 查询品种详情（过渡 API，供前端从 product_id 迁移到 variety）。

    内部根据 product_id 找到对应的 symbol，再返回 /api/varieties/{symbol}/detail 的等效数据。
    """
    ...
    v = db.query(VarietyDB).filter(VarietyDB.symbol == product.symbol).first()
    if not v:
        raise HTTPException(status_code=404, detail="关联的品种数据不存在")
