...
def detect_patterns(df: pd.DataFrame) -> dict[str, str]:
    ...
    if len(high_idx) >= 3:
        ...
        if h_vals[1] > h_vals[0] and h_vals[1] > h_vals[2]:
            if abs(h_vals[0] - h_vals[2]) / (h_vals[1] + 1e-10) < 0.03:
                pattern = "双顶（M头）"
    ...
    if len(low_idx) >= 3 and pattern == "未识别":
        ...
        if l_vals[1] < l_vals[0] and l_vals[1] < l_vals[2]:
            if abs(l_vals[0] - l_vals[2]) / (l_vals[1] + 1e-10) < 0.03:
                pattern = "双底（W底）"
