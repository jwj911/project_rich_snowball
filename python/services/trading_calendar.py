...
_BUILTIN_HOLIDAYS = {
    ...
    "10-01",
    "10-02",
    "10-03",  # 国庆节法定假日（10/4-7 为调休，已在 FLOATING 中维护）
}
...
_FLOATING_HOLIDAYS = {
    ...
    2026: [
        ...
        "06-19",
        "06-20",
        "06-21",  # 端午（修正：原硬编码 06-10 错误）
        "09-25",
        "09-26",
        "09-27",  # 中秋（修正：原硬编码 09-15~09-17 错误）
        "10-01",
        "10-02",
        "10-03",
        "10-04",
    ...
    2027: [
        ...
        "02-11",
        "02-12",
        "02-13",  # 春节（9天，除夕 2/5 逢周五）
        "04-03",
        "04-04",
        "04-05",  # 清明（预测：清明 4/5 周一，连上周末）
        "05-01",
        "05-02",
        "05-03",
        "05-04",
    ...
    2030: [
        ...
        "02-07",
        "02-08",
        "02-09",  # 春节（8天，除夕 2/2 周六）
        "04-05",
        "04-06",
        "04-07",  # 清明（预测：清明 4/5 周六，连上周末）
        "05-01",
        "05-02",
        "05-03",
        "05-04",
...
class TradingCalendar:
    ...
    def _load(self):
        ...
        if os.path.exists(json_path):
            ...
            logger.info(
                "Loaded trading calendar: %s dates, range %s ~ %s",
    ...
    def _json_path() -> str:
        return os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "trading_calendar.json"
        )
    ...
    def get_expected_kline_dates(
        self, start: date | datetime | str, end: date | datetime | str, period: str = "D"
    ) -> list[date]:
        """返回某周期下预期应有 K 线的日期列表。
