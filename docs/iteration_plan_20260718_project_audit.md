...
>
> 最新复核：2026-07-19，Phase 2 代码与 CI 收口提交已推送；Backend CI #22、
> Frontend CI #28（run `29670891119`）均已全绿。
...
| 项目 | 当前事实 |
| --- | --- |
| 工作区 | Phase 0/1/2 代码、测试和 CI 收口已提交并推送；Phase 2 已验收 |
| 后端源码 | 350 个 Python 文件 |
| 后端测试 | 85 个 pytest 文件 |
| 前端测试 | 33 个 Vitest 文件，6 个 Playwright spec |
| Alembic | 59 个迁移脚本，head 为 `f7a8b9c0d1e2` |
| 前端构建 | 通过，最大 First Load JS 为 `/products/[id]` 的 147 kB |
| 后端覆盖率 | 本地 `71.97%`，CI 门槛 `40%`；Backend CI #22 通过 |
...
| `npm run test` | **195 passed, 0 failed** | K 线、价位标注竞态和范围契约测试已同步 |
| `npm run build` | 通过 | 18 个路由生成成功 |
| 单独运行 `test_phase1_3_integration.py` | **26 passed, 0 failed** | schema 测试显式依赖建表 fixture |
| Backend CI #22 | **成功** | Alembic、PostgreSQL pytest、API smoke、Ruff、pip-audit 全部通过 |
| Frontend CI #28 | **成功** | PostgreSQL migration、backend、frontend build/start、Chromium Playwright、Vitest、Lighthouse 全部通过 |
...
| F-09 | **P2，Phase 2 已处理** | CI 门禁没有覆盖真实浏览器流程 | 初始审查时 [frontend-ci.yml](../.github/workflows/frontend-ci.yml) 只跑 Vitest、Build 和 Lighthouse，后端 coverage 阈值为 30%。 | 已增加 PostgreSQL + backend + Playwright Chromium smoke，并将 coverage 门槛提升到 40%；Backend CI #22、Frontend CI #28 均已通过。 |
...
### Phase 2：执行可靠性与生产拓扑（2026-07-18～19）— 已完成
...
- CI 增加依赖锁漂移检查、PostgreSQL API smoke、Playwright smoke，并将 coverage 门槛提升到 `40%`；
  前端 Test/Lighthouse 步骤显式固定在 `frontend/` 工作目录，并增加 Vitest 失败摘要与 artifact。
- 新增 `fut_main_daily_data` Alembic 迁移 `f7a8b9c0d1e2`，唯一键为
  `(variety_id, ts_code, period, trade_date)`，并增加 PostgreSQL schema 断言；
- 修复 `python-multipart`、`scikit-learn`、`starlette` 的已知漏洞版本，lock 与直接依赖保持一致。

验收结果：

- 本地后端：`965 passed, 8 skipped, 0 failed`；
- 覆盖率：`71.97%`；
- `docker compose config --quiet`、Python Ruff、TypeScript、ESLint、Next production build 和 Vitest 通过；
- Backend CI #22：Alembic、PostgreSQL pytest、API smoke、Ruff、`pip-audit` 全部成功；
- 本地定向 Playwright 品种详情 smoke：`3 passed`；完整 PostgreSQL/Chromium 远程验收由 Frontend CI #28 完成；
- Frontend CI #28：migration、backend、admin fixture、frontend build/start、Playwright smoke、Vitest、Lighthouse 全部成功；
- 价位标注 E2E 增加精确标题定位，避免技术分析文本变化导致 section locator 漂移；
- `usePriceLevels` 增加主力范围 `contract_id` 隔离、竞态响应保护、ID 删除和 optimistic mutation 合并。

对应提交：`7a235fd7`、`23a5aecc`、`60efd483`、`46ab2edc`、`210b7e82`、
`5874f78d`、`4e8dcf69`、`6c40a927`、`04e83fa7`、`bc2a519f`、
