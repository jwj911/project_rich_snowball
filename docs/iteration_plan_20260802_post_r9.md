...
> 计划日期：2026-08-02
> 当前状态：Post-R9 唯一当前迭代事实源；R10 待建立独立规格，尚无 Post-R9 工程项进入实施
...
本文只定义后续工作，不新增业务代码、数据库迁移、运行时配置或生产操作，也不生成生产报告。
其他项目入口的链接同步和活跃设计文档状态校正属于后续文档任务；在完成同步前，Post-R9 决策
仍由本文件统一管理。
...
`待立项`、`阻塞`、`未准入`、`未触发` 和 `候选` 是当前结论。只有独立规格批准且对应门禁满足后，
才可改为 `进行中`。
...
|---|---|---|---|---|---|---|
| R10 | CSP 脱敏证据归类与 S2 准入报告 | 可立即实施 | P0 | 待独立规格 | R9 已脱敏记录和受控发布元数据 | 有界只读能力、脱敏 JSON 和正确三状态判定 |
...
R10 是下一推荐工程迭代。目标是把 R9 已经脱敏的 `csp-violation` 记录转成可审计、可复现的
S2 评审输入，不收集新型敏感数据，也不修改任何 CSP。
...
R10 独立规格必须选择管理员鉴权的只读接口、离线只读命令，或同时提供两者，并满足：
...
- 先创建并批准 R10 独立规格；
- 独立规格固定输入 schema、受控枚举、四类硬上限、权限、保留、失败模式和测试矩阵。
...
下一步必须先为 R10“CSP 脱敏证据归类与 S2 准入报告”创建独立规格，固定输入 schema、受控
枚举、管理员/离线只读边界、查询窗口、扫描行数、输出大小、执行时间上限、三状态判定、JSON
契约、权限、保留、测试与停止条件。

在该规格获批前不新增 R10 代码；在 R10/R11 证据和人工评审完成前不实施 R12；在 R12 稳定
退出前不实施 R13。R8、R7 和三个产品候选继续保持未触发或未排期状态。
