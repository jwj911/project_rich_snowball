...
`待立项`、`阻塞`、`未准入`、`未触发` 和 `候选` 是门禁结论。R10 的“非生产工程基线已完成”
是工程交付状态，不代表生产部署或 S2 准入；其他事项只有独立规格批准且对应门禁满足后，才可
改为 `进行中`。
...
R10 是已完成本地与远端工程闭环的非生产迭代。目标是把 R9 已经脱敏的
`csp-violation` 记录转成可审计、可复现的 S2 评审输入，不收集新型敏感数据，也不修改任何
CSP。
...
- 本地 PostgreSQL 不可用，3 个 PostgreSQL 专项明确 skip，必须等待远端 PostgreSQL CI
  复核，不能用 SQLite 结果替代。该复核已由下述远端门禁完成；
...
- [Frontend CI run 30791923961（attempt 1）](https://github.com/jwj911/project_rich_snowball/actions/runs/30791923961)
  ...
  Lighthouse `home=92`、`products=100`。这是 `frontend/docs` 文档变更触发的远端回归，
  不构成生产部署、生产 SLO、完整业务周期或 S2 准入证据；
- Ruff、diff 和安全复核均通过；
