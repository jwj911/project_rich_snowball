...
> 最后更新：2026-07-19。本文档聚焦 `frontend/`，根目录 `README.md` 负责全栈启动与后端说明。
...
| 测试 | Vitest 4.1.7（33 个测试文件 / 192 个测试）+ jsdom + Testing Library；Playwright 6 个 spec；Backend CI #22 已通过 PostgreSQL/Alembic 后端 smoke |
...
Phase 0「可运行性收口」、Phase 1「行情读模型收敛」和 Phase 2「执行可靠性与生产拓扑」代码已完成：`npm run test` 为 `192 passed`，`npx tsc --noEmit`、`npm run lint`、`npm run build` 均通过，Playwright smoke 已加入 CI。Backend CI #22 已通过 PostgreSQL/Alembic、API smoke、Ruff 和 `pip-audit`；本次文档提交将触发 Frontend CI 的 PostgreSQL + Chromium smoke。前端 smoke 通过后进入文档发布治理和页面级覆盖扩展。
...
- 补登录/注册弹窗测试。
- 补评论提交、价位标注、自选切换测试。
- 引入 Playwright smoke：登录、行情中心、详情页、工作区。已加入 CI，Frontend CI 将在本次基线提交后重新验证远程 PostgreSQL/Chromium 流程。
